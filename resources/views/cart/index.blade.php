...
<div class="container">
    ...
    @if (count($items) < 1)
        <div align='center' style="padding-bottom: 20em; font: size 50px;">Nothing on cart </div>
    ...
    <table class="table">
        ...
        <tbody>
    <?php $count = 0; $qty=0; $tt = 0;
   
?>

    
    @foreach ($items as $item)
    <?php $count += 1;  $tt=(float)$tt+(float)$item->price
     ?>
    <tr>
    ...
    <tr>
      ...
      <td class="align-middle"><span style="font-weight:bold;color:red;">NPR.{{$tt}}</span></td>
      <td>
        <form action="https://uat.esewa.com.np/epay/main" method="POST" style="display: inline-block;">
          <input value="{{$tt}}" name="tAmt" type="hidden">
          <input value="{{$tt}}" name="amt" type="hidden">
      ...
      <td class="align-middle"> <a href="/product"><button  class="btn btn-success mt-1" style="width:305px;">CONTINUE SHOPING</button></a></td>
    </tr>
    </tbody>
    </table>
    @endif
